...
def conv_factory(x, nb_filter, img_dim, dropout_rate=None, weight_decay=1E-4):
    ...
    x = Activation('relu')(x)
    x = Conv2D(nb_filter, kernel_size=(int(img_dim[1]), int(img_dim[0]/4)),
...
def transition(x, nb_filter, img_dim, dropout_rate=None, weight_decay=1E-4):
    ...
    x = BatchNormalization(axis=2,
                           ...
                           beta_regularizer=l2(weight_decay))(x)
    x = Activation('relu')(x)
    x = Conv2D(nb_filter, kernel_size=(int(img_dim[0]/2),int(img_dim[1]/2)),
               kernel_initializer="he_uniform",
               padding="same",
               use_bias=False,
               kernel_regularizer=l2(weight_decay))(x)
    if dropout_rate:
        x = Dropout(dropout_rate)(x)
    x = AveragePooling2D((2,2), strides=2)(x) #try without!!!

    return x


def denseblock(x, nb_layers, nb_filter, growth_rate, img_dim,
    ...
    for i in range(nb_layers):
        x = conv_factory(x, growth_rate, img_dim, dropout_rate, weight_decay)
        list_feat.append(x)
...
def DenseNet(nb_classes, img_dim, depth, nb_dense_block, growth_rate,
    ...
    # Initial convolution
    x = Conv2D(nb_filter, kernel_size=(int(img_dim[0]),int(img_dim[1]/2)),
               kernel_initializer="he_uniform",
               padding="same",
               name="initial_conv2D",
               use_bias=False,
    ...
    for block_idx in range(nb_dense_block - 1):
        x, nb_filter = denseblock(x, nb_layers, nb_filter, growth_rate, img_dim,
                                  dropout_rate=dropout_rate,
                                  weight_decay=weight_decay)
        # add transition
        x = transition(x, nb_filter, img_dim, dropout_rate=dropout_rate,
                       weight_decay=weight_decay)
