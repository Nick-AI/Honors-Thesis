...
class Classifier:
    ...
    def get_embedding_matrix(text):
        embeddings = gensim.models.KeyedVectors.load_word2vec_format(self.embedding_file, binary=True)
        emebeddings = embeddings.wv
        text = self.clean_text(text)

    def run_embedding_classifier(self, t_depth=16, t_filters=12, t_growth_rate=12, in_dim=300, t_batchSize=512,
...
if __name__ == '__main__':
    denseNet = Classifier('Reuters', False, 125, 512)
    max_acc=0
    denseNet.run_embedding_classifier()
